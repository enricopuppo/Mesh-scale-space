...
std::tuple<vector<vec3f>, vector<vec3f>> path_positions_and_normals(
    const geodesic_path &path, const vector<vec3i> &triangles,
    const vector<vec3f> &positions, const vector<vec3i> &adjacencies) {
  auto pos = vector<vec3f>(path.lerps.size() + 2);
  auto nor = vector<vec3f>(path.lerps.size() + 2);
  pos[0] = eval_position(triangles, positions, path.start);
  nor[0] = tid_normal(triangles, positions, path.start.face);
  for (auto i = 0; i < path.lerps.size(); i++) {
    ...
    pos[i + 1] = p;
    nor[i] = tid_normal(triangles, positions, path.strip[i]);
  }
  pos.back() = eval_position(triangles, positions, path.end);
  nor.back() = tid_normal(triangles, positions, path.end.face);
  return {pos, nor};
}
...
vec2f tangent_path_direction(const shape_data &mesh,
                             const shape_geometry &geometry,
                             const geodesic_path &path, bool start) {
  ...
  if (start) {
    auto start_tr =
        init_flat_triangle(mesh.positions, mesh.triangles[path.start.face], 0);

    if (path.lerps.empty()) {
      auto p = interpolate_triangle(start_tr[0], start_tr[1], start_tr[2],
                                    path.end.uv);
      auto q = interpolate_triangle(start_tr[0], start_tr[1], start_tr[2],
                                    path.start.uv);
      direction = p - q;

    } else {
      auto x = path.lerps[0];
      auto k = find(geometry.adjacencies[path.strip[0]], path.strip[1]);
      auto p = lerp(start_tr[k], start_tr[(k + 1) % 3], x);
      auto q = interpolate_triangle(start_tr[0], start_tr[1], start_tr[2],
                                    path.start.uv);
      direction = p - q;
    }
    ...
    auto end_tr =
        init_flat_triangle(mesh.positions, mesh.triangles[path.end.face], 0);
    if (path.lerps.empty()) {
      auto p =
          interpolate_triangle(end_tr[0], end_tr[1], end_tr[2], path.end.uv);
      auto q =
      ...
      auto k = find(geometry.adjacencies[path.strip.rbegin()[0]],
                    path.strip.rbegin()[1]);
      auto p = lerp(end_tr[k], end_tr[(k + 1) % 3], 1 - x);
      auto q =
          interpolate_triangle(end_tr[0], end_tr[1], end_tr[2], path.end.uv);
...
vec3f tangent_path_direction3D(const shape_data &data,
                               const shape_geometry &geometry,
                               const geodesic_path &path, bool start) {

  auto direction = vec3f{};
  auto p0 = eval_position(data.triangles, data.positions, path.start);
  auto p1 = eval_position(data.triangles, data.positions, path.end);
  if (start) {
    ...
    if (path.lerps.empty()) {
      ...
      auto x = path.lerps[0];
      auto e = get_edge(data.triangles, data.positions, geometry.adjacencies,
                        path.strip[0], path.strip[1]);
      auto p = lerp(data.positions[e.x], data.positions[e.y], x);
      direction = p - p0;
    }
  } else {
    if (path.lerps.empty()) {
...
void p_transp_along_path(const shape_data &mesh, const shape_geometry &geometry,
                         const geodesic_path &path, vec3f v) {

  if (path.strip.size() == 1 && path.start.face != path.end.face)
    parallel_transp(geometry.angles, geometry.total_angles, mesh.triangles,
                    mesh.positions, geometry.adjacencies, geometry.v2t, v,
                    mesh.normals, path.start.face, path.end.face, T2T);
  else {
    for (auto i = 0; i < path.strip.size() - 1; ++i) {
      parallel_transp(geometry.angles, geometry.total_angles, mesh.triangles,
                      mesh.positions, geometry.adjacencies, geometry.v2t, v,
                      mesh.normals, path.strip[i], path.strip[i + 1], T2T);
    }
  }
}
// VTP
...
float intersect(const vec2f &start1, const vec2f &end1, const vec2f &start2,
                const vec2f &end2) {
  if (end1 == start2)
    return 0;
  if (end2 == start1)
...
vector<mesh_point> polthier_straightest_geodesic(
    const vector<vec3i> &triangles, const vector<vec3f> &positions,
...
vec3f flip_bary_to_adjacent_tri(const vector<vec3i> &adjacencies,
  ...
  bary1[k1] = bary0[(k0 + 1) % 3];
  bary1[(k1 + 1) % 3] = bary0[k0];
  bary1[(k1 + 2) % 3] = bary0[(k0 + 2) % 3];
...
handle_short_strips(const vector<vec3i> &triangles,
  ...
  if (strip.size() == 1) {
    {
      auto tid = start.face;
      assert(tid == strip[0] && tid == end.face);
...
geodesic_path compute_geodesic_path(const shape_data &data,
  ...
  //     data.triangles, data.positions, geometry.adjacencies, strip, start,
  //     end);
