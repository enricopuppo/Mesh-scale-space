...
//
//		where:
//			-upi scale			fig units per inch
//(default
//=
// 1200) 			-x low_x			x and y offset of upper
// left corner
//(inches) 			-y low_y			...(default = 1)
//-sz size maximum side length of figure (in inches)
//								...(default = 5)
//			-dx dim_x			horizontal dimension
//(default
//=
// 0) 			-dy dim_y			vertical dimension (default =
// 1) -sv value default slice value (default = 0)
//			-sl dim value		each such pair defines the value
// along the
//								...given dimension at
//which to slice.  This
//								...may be supplied
//for all dimensions except
//								...dim_x and
// dim_y. 			-ps pointsize		size of points in fig units
// (def = 10) 			file file (input=file.dmp, output=file.fig)
//
//----------------------------------------------------------------------
...
// is optional) 		0 <xxx> <xxx> ... <xxx>			(point
// indices
// and coordinates) 		1 <xxx> <xxx> ... <xxx>
//		  ...
//		tree <dim> <n_pts> <bkt_size>
//		<xxx> <xxx> ... <xxx>			(lower end of bounding
// box) 		<xxx> <xxx> ... <xxx>			(upper end of
// bounding
// box) 				If the tree is null, then a single line
// "null" is 				output.  Otherwise the nodes of the tree
// are printed 				one per line in preorder.  Leaves and
// splitting nodes 				have the following formats:
// Leaf node: 				leaf <n_pts> <bkt[0]>
//<bkt[1]> ... <bkt[n-1]> 		Splitting nodes:
//split <cut_dim> <cut_val> <lo_bound>
//<hi_bound> 		Shrinking nodes: 				shrink
//<n_bnds> 						<cut_dim> <cut_val>
//<side> 						<cut_dim> <cut_val> <side>
//						... (repeated n_bnds times)
//
//		Note: Infinite lo_ and hi_bounds are printed as the special
//				values "-INF" and "+INF", respectively.  We do
// not 				check for this, because the current version of
// ANN starts with a finite bounding box if the tree is nonempty.
//----------------------------------------------------------------------
