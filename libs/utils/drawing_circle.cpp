...
Isoline create_isoline(const vector<vec3i> &triangles,
  ...
  auto d = vector<float>(d0.size());
  for (auto i = 0; i < d0.size(); ++i)
    d[i] = d0[i] - d1[i];

  return create_isoline(triangles, adjacencies, d, 0.f);
}
...
Circle create_circle(const shape_data &mesh, const shape_geometry &geometry,
  ...
  circle.distances = compute_geodesic_distances(
      solver, mesh.triangles, mesh.positions, geometry.adjacencies, {center});

  if (!set_radius(mesh.triangles, geometry.adjacencies, &circle, radius)) {
    return Circle();
  }
