...
pair<bool, vec2f> point_in_triangle(const vector<vec3i> &triangles,
                                    ...
                                    const int tid, const vec3f &point,
                                    float tol) {
...
unfold_triangle init_flat_triangle(const vector<vec3f> &positions,
                                   const vec3i &tr, const int k) {
  unfold_triangle tr2d;
  auto l = length(positions[tr[k]] - positions[tr[(k + 1) % 3]]);
  tr2d[k] = vec2f{0, 0};
  tr2d[(k + 1) % 3] = vec2f{l, 0};
  auto rx = length_squared(positions[tr[k]] - positions[tr[(k + 2) % 3]]);
  auto ry =
      length_squared(positions[tr[(k + 1) % 3]] - positions[tr[(k + 2) % 3]]);
...
// assign 2D coordinates to a strip of triangles
vector<unfold_triangle> unfold_strip(const vector<vec3i> &triangles,
                                     const vector<vec3f> &positions,
                                     const vector<vec3i> &adjacencies,
                                     const vector<int> &strip, const int tid) {
  auto coords = vector<unfold_triangle>(strip.size());
  coords[0] = init_flat_triangle(positions, triangles[tid], 0);
...
bool is_boundary(const vector<vec3i> &adjacencies,
                 const vector<vector<int>> &v2t, const int vid) {
  auto star = v2t[vid];
  for (auto tid : star) {
    auto adj = adjacencies[tid];
    for (auto i = 0; i < 3; ++i) {
      if (adj[i] == -1)
        return true;
    }
...
geodesic_solver make_geodesic_solver(const vector<vec3i> &triangles,
  ...
  for (auto i = 0; i < positions.size(); ++i) {
    ...
    if (is_boundary(adjacencies, v2t, i) || star.size() == 0)
      continue;
...
vector<vector<float>> compute_angles_wo_opposite(
  ...
  for (auto i = 0; i < positions.size(); ++i) {
    ...
    for (auto tid : v2t[i]) {
      auto k = find_in_vec(triangles[tid], i);
      auto v =
          positions[triangles[tid][(k + 1) % 3]] - positions[triangles[tid][k]];
      auto w =
          positions[triangles[tid][(k + 2) % 3]] - positions[triangles[tid][k]];
      curr_angles.push_back(angle(v, w));
      theta += angle(v, w);
    }
    auto scale_factor = 2 * pif / theta;
    total_angles[i] = theta;
...
unfold_triangle unfold_face(const vector<vec3i> &triangles,
  ...
  auto r1 = length_squared(positions[v] - positions[b]);
  result[mod3(j + 2)] =
      intersect_circles(result[j], r1, result[mod3(j + 1)], r0);
#else
  // new method
  auto &pa = positions[a];
  auto &pb = positions[b];
