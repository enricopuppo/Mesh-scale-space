...
// void set_points_shape(ogl_shape *shape, const shape_data &mesh,
...
Added_Path *add_path_shape(App &app, const vector<vec3f> &positions,
  ...
  auto added_path = new Added_Path{};
  added_path->color = color;
  added_path->radius = radius;
  added_path->positions = positions;
  auto shape = add_shape(app.scene);
  ...
  added_path->instance =
      add_instance(app.scene, identity3x4f, shape, material, false);
  app.added_paths.emplace(app.added_paths.begin() + entry, added_path);
  return added_path;
}
void show_shape(App &app, const int entry) {
  if (entry > app.added_paths.size()) {
    std::cout << "No valid entry" << std::endl;
    return;
  }
  auto &path = app.added_paths[entry];
  auto shape = add_shape(app.scene);
  auto material = add_material(app.scene, {0, 0, 0},
                               path->instance->material->color, 1, 0, 0.4);
  update_path_shape(shape, app.mesh, path->positions, path->radius);
  path->instance =
      add_instance(app.scene, identity3x4f, shape, material, false);
}
...
Added_Path *add_polygonal_shape(App &app, const vector<mesh_point> &vertices) {
  ...
  if (validate_points(vertices)) {
    ...
    return add_path_shape(app, pos, app.curve_size * 0.0002, app.curr_color);
  }
  return nullptr;
}
...
void update_polygonal_shape(App &app, const vector<mesh_point> &vertices) {
  ...
  if (validate_points(vertices)) {
    ...
    update_path_shape(app.canv.result_shape->instance->shape, app.mesh, pos,
...
void update_polygonal_shape(App &app, const vector<mesh_point> &vertices,
  ...
  if (validate_points(vertices)) {
    ...
    update_path_shape(app.added_paths, app.mesh, pos, app.curve_size * 0.0002,
                      entry);
  }
}
...
add_result_shape(App &app, const vector<mesh_point> &vertices) {
  ...
  result = add_polygonal_shape(app, vertices);
...
inline mesh_point intersect_circle_point(App &app, const vec2f &mouse) {
  ...
  for (auto &curve : app.selected_circle->isoline) {
    auto curve_pos =
        closed_curve_positions(curve, app.mesh.triangles, app.mesh.positions,
                               app.topology.adjacencies);
      ...
      if (dist < min_dist) {
        closest_point =
            get_closed_curve_point(app.mesh.triangles, app.mesh.positions,
                                   app.topology.adjacencies, curve, i);
...
void compute_affine_transformation(App &app, mesh_point &point,
  ...
  if (app.input().rotating) {
    ...
    if (app.selected_circle->construction == -1) {
      ...
      auto path = compute_geodesic_path(app.mesh, app.topology, app.dual_solver,
                                        app.selected_circle->center, point);
      auto t = tangent_path_direction(app.mesh, app.topology, path, true);
      auto e = vec2f{1, 0};
      app.selected_circle->theta = angle(e, t);
      if (cross(e, t) > 0)
        app.selected_circle->theta = 2 * pif - app.selected_circle->theta;
    }
  } else if (app.input().translating) {
    auto path = compute_geodesic_path(app.mesh, app.topology, app.dual_solver,
                                      app.selected_circle->center, point);
    auto &theta = app.selected_circle->theta;
    auto t0 = tangent_path_direction(app.mesh, app.topology, path, true);
    auto t1 = tangent_path_direction(app.mesh, app.topology, path, false);
    t1 *= -1;
    auto tr_end = init_flat_triangle(app.mesh.positions,
                                     app.mesh.triangles[path.end.face], 0);

    auto tr_start = init_flat_triangle(app.mesh.positions,
                                       app.mesh.triangles[path.start.face], 0);

    auto e_start = normalize(tr_start[1]);
    auto e_end = normalize(tr_end[1]);

    auto phi0 = angle(e_start, t0);
    if (cross(e_start, t0) > 0)
      phi0 = 2 * pif - phi0;

    auto psi = theta - phi0;
    auto phi1 = angle(e_end, t1);
    if (cross(e_end, t1) > 0)
      phi1 = 2 * pif - phi1;

    app.selected_circle->theta = phi1 + psi;
    app.selected_circle->center = point;
    app.selected_circle->distances = compute_geodesic_distances(
        app.solver, app.mesh.triangles, app.mesh.positions,
        app.topology.adjacencies, {point});
    set_radius(app.mesh.triangles, app.topology.adjacencies,
               app.selected_circle, app.selected_circle->radius);
  } else if (app.input().scaling) {
    if (app.selected_circle->construction == -1) {
      set_radius(
          app.mesh.triangles, app.topology.adjacencies, app.selected_circle,
          get_distance(app.mesh.triangles[point.face], get_bary(point.uv),
